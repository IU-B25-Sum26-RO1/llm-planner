...
| `SIM-01` | Task | Control | M | `WIN-02` | Add a headless Gazebo smoke test for `go_home`, `open_gripper`, `close_gripper`, `pick pyramid_cube_1`, and `place table`. | Automated test records action terminal state, timeout, final arm error, held-object state, and final Gazebo model pose; all five cases pass three seeded runs. |
