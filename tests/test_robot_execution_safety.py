...
def test_pick_reports_failure_when_virtual_attachment_fails(monkeypatch):
    ...
    success, error = interface._pick("pyramid_cube_1")
...
def test_action_server_rejects_invalid_goal_requests(monkeypatch):
    ...
    accepted = interface.goal_callback(
        types.SimpleNamespace(
            task_type="pick", object_name="pyramid_cube_1", x=0.0, y=0.0, z=0.0
        )
    )
    accepted_coordinates = interface.goal_callback(
        types.SimpleNamespace(
            task_type="pick", object_name="", x=0.25, y=-0.1, z=0.9
        )
    )
    ...
    assert accepted_coordinates == "accept"
...
def test_place_requires_a_held_object(monkeypatch):
    ...
    success, error = interface._place("table", None)
